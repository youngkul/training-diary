...
async function loadAllVideos() {
  ...
  const session = await getSession();
  const currentUid = session?.user?.uid;
  const renderedVideos = new Set(); // ✅ 중복 방지용
  ...
  for (const docSnap of snapshot.docs) {
    ...
    if (renderedVideos.has(video.id)) continue;
    renderedVideos.add(video.id);
    ...
    videoDiv.innerHTML = `
      <div class="bg-white rounded-2xl shadow-lg p-5 space-y-4">
        ...
        <video src="${video.url}" poster="${video.poster || 'https://placehold.co/640x360?text=썸네일'}" controls muted playsinline preload="metadata" loading="lazy" class="w-full aspect-video rounded-xl shadow-lg border border-gray-200"></video>
    ...
    // ✅ 병렬 처리
    Promise.all([loadComments(video.id), loadLikes(video.id)]);
  }
}
